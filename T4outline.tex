...
\begin{itemize}
   ...
   \item  Test how well we can model and capture protein conformational change in the T4 Lysozyme L99A apolar cavity (?) %Maybe same as above
   \item Accurately predict relative binding free energies while sampling protein conformational changes. %Seems like a possible outcome and less an objective
...
%DLM: This last is a great and careful take-away sentence; it's one that will both fly with Schrodinger AND probably makes our point well.

%DLM: I think the above subsection is largely on the right track. There's a lot of wordsmithing I would do (which can come later) but I generallly like the content. The one main content point I would make at this point is that I think you want to start off the subsection with saying what you're getting at in a less technical and more direct way that sort of cuts to the chase. For example, maybe something like, "Here, we found that relative free energy calculations can suffer from substantial convergence problems resulting from relatively modest protein conformational changes, and these problems have profound implications for the accuracy of computed relative free energies in these cases. Particularly, we find that calculated relative free energies depend on the initial protein conformational state by up to X kcal/mol, even with simulations of up to Y ns per lambda window, far more than is typically used in the XYZ standard protocol for these calculations." 
%Word-smithing wise, be sure to see the Plaxco science writing paper if you've not already read it: https://www.dropbox.com/s/hmxdcl100wdf9dr/Plaxco_-Protein%20Science.pdf?dl=0
%Caitlin found the suggestions there useful enough - particularly the one about each paragraph having a point and the first sentence summarizing it - that she started writing comments into her LaTeX where there is a one-liner before each paragraph which summarizes what the paragraph is supposed to do. YMMV, but the general idea is very helpful for writing. 
...
%DLM: Again, content so far is good but may need summary statements. One thing that tends to make me lose interest is when papers get too much into technical details without keeping me aware of the big picture, which is a danger you're running here. You may want a summary statement at the top of this subsection which orients people, i.e. perhaps following after your opening "In order to..." statement (which is good), you might say something like, "This was expected to... and indeed we found that this improved ... but was still inadequate [or whatever] because we still found a dependence of up to ... depending on the starting protein conformational state, even with simulations of up to... This seemed to be particularly due to .... To be more specific, we examined (get into existing stuff)." Though, to some extent, some of your detailed results discussed here may belong in results rather than in discussion (?). 
...
Thus, we cannot say these are converged; a result from inadequate sampling of all the protein conformational states. %DLM: the "a result from..." reference is unclear. Do you mean "this poor convergence results from..."?
...
%DLM: This is seeming overly detailed for a discussion section. Maybe this is a "results and discussion" section? Or, "Analysis and discussion"? I see how you have it currently laid out there is not an earlier "Results" section (though the binding affinities are already given) so maybe the title of this section needs broadening.
%DLM: As mentioned in person, this is NOT a problem with what you've written, it's just a question of whether this much detail belongs here or somewhere else. I also mentioned imagining how you'd react to this level of detail being provided in a talk about a system with which you weren't familiar. 
%DLM: It may be that this is when you need to identify a target journal, as some will give more leeway in terms of deciding what headers to use than others. Ideas: JCTC, J Comp Chem, JACS (probably would reject it due to being "not of general interest", though we could argue it's appropriate given the impact of the earlier Schrodinger article), etc. Open for input. Or J. Phys. Chem. B (Adrian Roitberg is an editor?)

***INSERT DATA FROM 55ns pREST simulations***\\ %DLM: Belongs in results?
...
It is much easier to do so by performing smaller perturbations and/or running longer simulations. %DLM: How does ``smaller perturbations'' help? Isn't the problem an issue more of "at what point does the protein start to change conformation?" rather than "how big is the perturbation?" i.e. maybe just one methyl addition can be a problem if it's the right methyl, right?
...
Thereby, requiring much longer simulation times or additional computational power, which are not desirable factors when working with large ligand libraries. %Grammar, not a complete sentence.
