...
Contains the reference protein-ligand crystal structures that have been aligned and prepared through the `Protein Preparation Wizard' tool.
\item [RMSD analysis:]\texttt{scripts.tar.gz}\\
Contains the scripts \texttt{RMSD-analysis.tcl} and \texttt{plot-RMSD.py} used for RMSD analysis. A \texttt{README.txt} file has been provided which details how to generate the RMSD/time and Colormap plots found in the paper. VMD and the trajectories from the MD simulations are required to perform the analysis. Only trajectories for benzene-nhexylbenzene and benzene-nhexylbenzene simulations have been included and are within the archived data files.
...
Inside each archived directory contains directories named by the ligands involved in the transformation and the protein conformation from which the simulations were started with. For example, `\texttt{C-O\_default/benzene-nhexylbenzene\_C/}' corresponds to the FEP calculation of benzene to n-hexylbenzene, starting from the protein closed state, while using the default protocol. Within each FEP calculation directory are the input \texttt{.mae} files containing the protein/ligand structures, the simulation configuration \texttt{.msj} files, and a \texttt{README.txt}. The \texttt{README.txt} file contains information about each FEP calculation such as the protein starting conformation, ligands involved, forcefields used, set label, free energies corresponding to the complex/solvent simulations, and the final calculated free energy. The output data for each FEP calculation is contained within the `\texttt{*\_fep1/}', where inside the `\texttt{*\_complex\_12/}' or `\texttt{*\_solvent\_13/}' folders are all output files used to calculate the free energies. The final calculated free energy is written in a `\texttt{result}' file, where extended simulations will contain two result files for the calculated free energy when analyzing the 0-5ns time frame and the 15-25ns or 40-55ns time frame.\\ 
For FEP calculations where an RMSD analysis was performed in the paper (ex.`\texttt{C-O\_default/benzene-nhexylbenzene\_C}'), an `\texttt{RMSD-analysis/}' folder is provided. Inside are `\texttt{replica\#.rmsd}' files that contain the set of RMSDs relative to each protein conformation over each replica's trajectory. The resulting RMSD/time and Colormap plots can be found inside the `\texttt{RMSD-analysis/plots/}' folders. The trajectories are also provided inside the output `\texttt{*\_fep1/}' folder within the `\texttt{*\_complex\_10/}' directory. \texttt{C-O\_pREST.tar.gz} has been split due to file size limitations. It can be rejoined by executing `\texttt{cat C-O\_pREST.tar.gz.* $>$ C-O\_pREST.tar.gz}'
\end{description}
The Supporting Information files can be downloaded at http://n2t.net/ark:/b7280/d1js3b (doi:10.7280/D1JS3B).
\end{suppinfo}
