...
%P3: Speculation on best practices for other future studies
%NML: I think these first 3 sentences might be a bit of repetition of the first paragraph? Include or toss?
%From our study conducted here, we present strong evidence for the importance of studying kinetically distinct protein conformational states prior to performing binding free energy calculations.
...
In summary, solute molecules are restrained to their initial positions while minimizing using the Brownie integrator\cite(???) for 100ps, followed by 12ps simulations at 10K with a NVT ensemble and then a NPT ensemble using the Langevin method \cite(???).
...
Further details on the temperature profile and how the REST region is selected can be found in previous studies\cite{FEP/REST,FEPplus} in the supporting information.
