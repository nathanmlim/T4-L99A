...
\section{Results}
%DLM: Sometimes I like to use subsection headers in the "titles should be statements" vein, i.e. "Dependence on protein starting conformation" could be "Calculated free energies depend strongly on starting protein conformation", and so on. 
...
%DLM: Discussion looks pretty good. Some of same wording suggestions from conclusions may apply. 
%DLM: You might also want a paragraph speculating about implications for other studies  - i.e., if I gave you "drug discovery project x on unstudied protein Y" which only had an apo structure available, what would you do? This is a good opportunity to say forward looking best-practices type things, like, "It might be necessary to check..." or "MSMs could be helpful for...", etc. This work is important not just because of what it says about lysozyme or sampling when you have protein conformational changes, but what it might imply about what people should be doing when they do drug discovery/attempt to apply Schrodinger's recommendations to real problems. So if you had to do that, what would you do, in view of your results here? (I'm happy to comment/speculate too, but I want you to try first...:) )
...
%DLM: Identification isn't an essential factor, I don't think - i.e. if there ARE multiple states, it doesn't necessarily matter if you've identified them, right? I think you mean something more like, "Overall, we have shown that the presence of kinetically distinct protein conformational states that are modulated by ligand binding can dramatically impact the accuracy of these calculations," or something like that? There is probably even a better way of saying it but I have to rush.
...
%DLM: This is true even if we do know of the different states, right? It's just that we detected it because we had these different states... 
Even more limiting, would be the case of only having the apo protein structure available, such as common in early drug discovery phases for new therapeutic targets. %Minor grammar. Maybe, "In this study, we would have encountered a worst-case scenario if we only had the structure of the \emph{apo} protein available, as is often the case in early stage drug discovery. Then, ... "
Dangerously, a medicinal chemist could dock a library of ligands and run free energy predictions where the chemist would then discard ligands with apparently low binding affinities which appear unfavorable only because of unsampled protein conformational changes.
Without prior knowledge of the existence of other protein conformational states, one would never know that these calculations are actually incorrect due to poor convergence.
%DLM: I like the point you're making in this paragraph above. The main thrust of my comments is to try and clean up the grammar, make sure the sentences are really saying what you intend, and make it very very clear. (Probably even more can be done than I've suggested.)
...
In this study, we show that only including ligand atoms into the REST region and using the typical 5ns simulation time  %DLM: I'd change the order of clauses, i.e. "show that using a typical 5 ns simulation time with only ligand atoms in the REST..." The REST aspect is a more technical point and so I think it makes sense to follow with that point rather than leading with it. 
leads to free energies that significantly depend on the initial protein conformational state. 
Only by expansion of the REST region to include key protein residues and longer simulation times %DLM: Again, switch order? 
were we able to eliminate the dependence on the starting conformation and come close to convergence.
...
Both FEP protocols use the default Desmond relaxation protocol, described previously \cite{FEPplus} in the supporting information, and the FEP/REST methodology \cite{REST,REST2,FEP/REST,FEP/RESTapp}. %DLM: I suggest spending a sentence summarizing the default relaxation protocol (or two sentences if necessary), i.e. how much of what type of equilibration, etc. Later: I actually see a summary of this (?) below itemized, which is probably sufficient. Just point to it here? 
...
Simulations were carried out to a%n arbitrarily chosen %DLM: If you don't have a good reason for the length, best is probably just not to say. "Arbitrarily chosen" sounds sort of like, "we just made it up", and invites criticism. It's fine to just choose arbitrarily, of course, and if you don't say why you did it, people will assume it was arbitrary. So... 
length of up to 55ns for closed-open transformations and up to 25ns for closed-intermediate transformations.

In this study, by default, only heavy atoms in the ligand were included in the REST region unless specified otherwise.
Further details on the temperature profile and how the REST region is selected have been discussed previously\cite{FEP/REST,FEPplus} in the supporting information.
...
%DLM: Note curly quotes are achieved in tex using ` and ' rather than ' '. So `pREST' would be what you want above. 

For `pREST' simulations, Based on visual inspection of our molecular dynamics simulations and considering the F-helix spans residues 107-115, we selected residues Glu108, Val111, and Gly113 to include into the REST region (Fig~\ref{fig:C2O}).
Near the start of the helix sits Glu108 %DLM: Order - usually "Glu 108 sits..." 
 which appears as a hinge point for the opening and closing of the binding cavity (Fig~\ref{fig:Glu108-C2O}).
...
%DLM: This confuses me. If you used all of the above eight or nine protein structures, but you started simulations from just two, what were the other protein structures used for? 
...
%DLM: Confusing to say "not docked via Glide" as it suggests there was something you DID use Glide for. This is the only reference to Glide in the paper, so unless you DID use it for something, you should probably remove it. Just, "not overlaid or docked..." is probably sufficient.
...
All proteins were prepared and aligned in Maestro \cite{Maestro} using the 'Protein Preparation Wizard' \cite{ProteinPrepWizSoftware,ProteinPrepWizPaper} tool and the following settings enabled:
   \begin{itemize}
   \item Preprocess: Assign bond orders, Add hydrogens, Create zero-order bonds to metals, Create disulfide bonds, Cap termini, Delete waters beyond 5\AA  from het groups %DLM: "het groups"? Heteroatoms? Or another better name? 
