...
Using alchemical methods like free energy perturbation (FEP), thermodynamic integration, and $\lambda$-dynamics on molecular dynamics simulations, the difference in binding free energies between two ligands can be computed in a robust and accurate manner.
...
Although the previous report on FEP+\cite{FEPplus} demonstrates its robustness, a primary concern focuses on the size of the ligand perturbation or the initial ligand pose and how these factors impact the accuracy of calculated free energies.\cite{mobley2012perspective,doi:10.1021/acs.jctc.5b00214}
...
However, the level of discrepancy is an 0.6 kcal/mol RMSI, which may or may not be problematic depending on the application. 
...
%DLM: Discussed with RA and the point he was trying to make was not that the results are so accurate here, but rather that in OTHER tests, experimental noise may lead the results to look worse than they are. Trying to add that here. 
...
These problems have profound implications for the accuracy of computed relative free energies in these cases. Particularly, we find that calculated relative free energies can depend on the initial protein conformational state by up to 4 kcal/mol.
...
It may be comforting that even without this prior knowledge, good to very good agreement with experiment is obtained for the majority of ligands. However, one should keep in mind that this study was carefully designed to probe the potential difficulties caused by these conformational changes; most lysozyme ligands studied previously do not induce such significant conformational changes. 
