...
However, despite these recent innovations, there are still needs for further improvement in sampling algorithms to more adequately sample protein motion relevant to protein-ligand binding. Here, we report our work identifying and studying such clear and remaining needs in the apolar cavity of T4 Lysozyme L99A. %DLM: Swapped in rephrasing of RA edits.
...
Although the previous report on FEP+\cite{FEPplus} demonstrates its robustness---like much of the FEP literature---a primary concern focuses on the size of the ligand perturbation or the initial ligand pose and how these factors impact the accuracy of calculated free energies.\cite{mobley2012perspective,doi:10.1021/acs.jctc.5b00214}
...
Some computational studies have shown even small simple changes in the protein---like a side-chain rotamer flipping---can cause large errors in affinity predictions if not sampled\cite{Mobley2009489,Mobley20071118} and thus protein conformational changes should be carefully sampled.
In the previous FEP+ study\cite{FEPplus}, many of the protein targets used were either extremely rigid (i.e. tyk2 and thrombin) or ligand pairs often avoid issues relating to protein conformational changes.
Potentially, this could mean the domain of applicability for the FEP+ protocol is restricted to either extremely rigid proteins or ligand pairs that do not involve a significant protein conformational change.
Since it is extremely common to find that ligand binding causes the protein to adopt a different conformation, this limits the range of projects free energy calculations can be used for in industrial drug discovery projects despite their potential accuracy.
Furthermore, protein-ligand binding can be more complex than this, with proteins potentially adopting multiple conformations even for the same ligand \cite{doi:10.1021/jm060167o,Gutteridge200521,Merski2015}.
Thus, there is a great need to understand how protein conformational changes and the choice of starting protein conformation affects the accuracy of free energy predictions, so that is our focus here. 
...
Through this observation, we demonstrate further that the default protocol does not eliminate the free energy dependence on the protein starting conformation, even for smaller perturbations. 
However, the level of discrepancy is much more comparable to the typically expected sampling error for routine scoring work. %DLM: We don't really agree with this point, as it depends on what people expect and what their transformation size is. WE don't expect a 0.6 kcal/mol convergence error. Suggest we cut, or use something like "However, the level of discrepancy is an 0.6 kcal/mol RMSI which is not extremely alarming."
...
%Interesting, the accuracy versus experiment of using either starting structure with the default protocol is well within the earlier published estimates, and exceptional accuracy is obtained using the open structure, comparable to the experimental accuracy of most common experimental binding assays. %DLM: Issues with this sentence; I'm not sure what it means to be "well within the earlier published estimates", and there's that "exceptional accuracy" term again; also, the accuracy of "most common experimental binding assays" is not really relevant here since this is high quality ITC data. PING ROBERT ON THIS WORDING.
...
%It should be noted that this accuracy is comparable to, or possibly better than, the accuracy of many assays used in drug discovery campaigns by pharmaceutical companies. %DLM: This doesn't seem relevant to us and we prefer to cut it; we can retain if he insists but would want to add disclaimer that this is not relevant to the present data. 
 
\section{Discussion}
In this study, we find  that relative free energy calculations for this system can suffer from substantial convergence problems resulting from relatively modest protein conformational changes.
...
These problems have profound implications for the accuracy of computed relative free energies in these cases. %DLM: We would prefer this - a 4 kcal/mol discrepancy is a "profound implication". 
...
%Interestingly, despite the slow degrees of freedom present in the system, the accuracy of the default protocol is still in line with earlier reported work, and exceptionally accurate if the more open crystal structure is initially used. %DLM: We want to back off from this a little bit. The issue here is discrepancy depending on starting protein structure, not accuracy relative to experiment. Plus, accuracy measures ignore the cases with the largest inconsistency! 
%DLM version:
Interestingly, despite the slow degrees of freedom present in the system, the accuracy of the default protocol is still in line with earlier reported work, especially beginning with the open crystal structure (though this ignores the cases where we observed the largest inconsistencies, since no experimental affinities were available in those cases).
...
%However, when we begin with the protein open state our $\Delta\Delta G_{calc}$ values have deeply surprising agreement with experiment, despite our inability to sample the closed state. %DLM: We don't like this at all. They come out overly favorable (slightly) in the cases where we have data, and clearly much too overly favorable in n-butyl/n-hexylbenzene cases (anything which has predominantly open). %GO BACK TO ORIGINAL WORDING. 
...
On the other hand, if only the open protein-ligand complexes were available, the opposite would be concluded in that larger ligands are better binders than smaller ligands. %DLM: We want to retain this sentence; this is absolutely what Table 5 shows conclusively.
We speculatively note, this may be problematic for fragment based drug discovery campaigns, but is somewhat unlikely for high-throughput screening campaigns, where most of the screened molecules will be highly elaborated. %DLM: This requires assuming this is only an effect when you go from small to big, but there is no reasno to believe it has to be limited to that. Bring up with RA.
...
%Remarkably, the absolute agreement with experiment of the default protocol ranges from within the previously reported accuracy of 1.0 kcal/mol, to exceptionally accurate at 0.58 kcal/mol, despite such a slow degree of freedom being present in the system. %DLM: Really only applies when starting from open, to the ligands with known affinities (which means these statistics ignore the cases with the large conformational change!). Back off on accuracy claims as these are ignoring the cases with biggest inconsistencies. 
...
%This following paragraph is false, typically a holo structure is available much earlier, apo structures are much less common.  I've commented out the paragraph. 
