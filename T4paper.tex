...
In this study, root-mean-square-deviation (RMSD) of the backbone atoms in the F-helix are used to determine the conformational state of the protein over the course of the trajectory.
From our RMSD analysis, we demonstrate the standard 5ns simulation time and default REST region selection are insufficient to adequately sample the motion in the F-helix and eliminate the dependence on the initial protein state.  
...
From experimental data of ligand occupancies (Table~\ref{tbl:loop-occ}), we expect in our simulations of n-hexylbenzene to see the protein primarily in the open state and fewer in the closed state.
...
As the protein strains to accommodate n-hexylbenzene and enter its preferred open state, protein-ligand strain results and is reflected in the positivity of the calculated $\Delta\Delta G$(+2.4 kcal/mol)
...
Expectedly, the calculated $\Delta\Delta G$ comes out negative(-0.6 kcal/mol) as there is no occurrence of large protein-ligand strain in order to open the cavity.
Ultimately, we arrive at two very different relative free energies, where the discrepancy is a whopping +3.0 kcal/mol, for the same transformation of benzene to n-hexylbenzene\footnotemark.
Despite the use of FEP/REST, it is clear that we are unable to adequately sample all the relevant states within the 5ns time frame, resulting in such a large difference in calculated $\Delta\Delta G$.
...
\section{Figures}
\begin{figure}
   \frame{\includegraphics[trim={1.5cm 0 2cm 0.25cm}, clip, width=\textwidth]{VMDscripts/c_opls3_1/plots/0-5ns/RMSD-replica11.png}}
   \caption{Closed - Benzene to n-Hexylbenzene 0-5ns RMSD Replica11}
   \label{fig:c_opls3_1/RMSD-replica11}
\end{figure}

\begin{figure}
   \frame{\includegraphics[trim={1.5cm 0 2cm 0.25cm}, clip, width=\textwidth]{VMDscripts/o_opls3_1/plots/0-5ns/RMSD-replica11.png}}
...
\begin{tabular}{|c|c|c|c|}
\hline
\textit{\textbf{PDB}} & \textbf{Ligand}                          & \textbf{\begin{tabular}[c]{@{}c@{}} \boldmath$\Delta G_{exp}$ \\ (kcal/mol)\end{tabular}} & \textbf{Error} \\ \hline
