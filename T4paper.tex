...
\footnotetext{We introduce this RMSI term in order to distinguish it from other similar terms like RMS-Error (RMSE) which would refer to comparisons with $\Delta\Delta Gs_{exp}$ or RMS-Deviation (RMSD) which we use in the context of tracking the helix motion and determining the protein state during a simulation.}

\subsection*{Determining the protein conformation state using RMSD}
In this study, we determine the state of the protein by computing the root-mean-square-deviation (RMSD) of the protein backbone atoms spanning the F-helix relative to their positions found in the closed (PDB:4W53), intermediate (PDB:4W57), and open (PDB:4W59) crystal structures.
...
Here, we use VMD \cite(???) to align and compute the RMSD of our Desmond trajectories to the crystal structures.
Further details on the procedure and the scripts used for these analyses can be found in the supplementary info.
...
We apply the RMSD/time analysis only to the simulations that are physically relevant or in other words the end-state replica (i.e. $\Lambda_0/\Lambda_{11}$).
...
Additionally, we can gain some insight on the time required to capture the opening or closing of the binding cavity.

It is important to note that by limiting our view to only the end-states, we encounter time points that may appear contradictory, at a glance. 
For example, between 3-4ns there are two frames where the protein is closed and the RMSD is slightly greater than 2.0\AA.
Following, there is one frame where the protein is open but the RMSD is lower than 2.0\AA.
This appears to be an artifact of coordinate swapping during replica exchanges from other lambda windows, resulting in apparently contradictory time points.

We address this limitation by viewing all replicas in what we call a color map, see Figure~\ref{fig:c_opls3_rest1_1/colormap} for an example.
In this analysis, we color time points according to the lowest RMSD state for all replicas but do not track the RMSD relative to the closed state as intermediate lambdas are not physically relevant.
Instead, we gain a better view of the overall protein conformational sampling through a collective view of all replicas and the states they occupy over their separate trajectories. 
