...
\title{Sensitivity in relative binding free energies from incremental ligand changes within a simple model binding site using modern free energby calculation protocols and force fields}
...
This was done in an attempt to correct bond angles and minimize the core RMSD, which LigandFEP uses to determine the core atoms between the two ligands (LW?)
%which LigandFEP uses to select the ligand heavy atoms to be included in the REST region. 
...
FEP/REST simulations were run on four GeForce GTX Titan Black GPUs using the Desmond/GPU engine with OPLS3\cite{OPLS3} forcefield parameters.
Additional simulations were run using the OPLS2005\cite{OPLS2005} and the corresponding data can be found in the supplementary information.
...
%LW: HOW CAN I CLARIFY THIS A BIT?
...
On the other hand, for closed-intermediate, the RMSI raises slightly to +0.78 kcal/mol (previously +0.60 kcal/mol), which may be due to statistical noise.
