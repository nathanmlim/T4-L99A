...
\begin{description}
\item [RMSD analysis scripts:]\texttt{RMSD-analysis.tcl} and \texttt{plot-RMSD.py}\\
A \texttt{README.txt} file has been provided in the `\texttt{scripts/}' directory that details how to generate the RMSD/time and Colormap plots found in the paper. VMD and the trajectories from the MD simulations are required to perform the analysis. Only trajectories for benzene-nhexylbenzene and benzene-nhexylbenzene simulations have been included and are within the archived data files.
\end{description}   
...
For FEP calculations where an RMSD analysis was performed in the paper (ex.`\texttt{C-O\_default/benzene-nhexylbenzene\_C}'), an `\texttt{RMSD-analysis/}' folder is provided. Inside are `\texttt{replica\#.rmsd}' files that contain the set of RMSDs relative to each protein conformation over each replica's trajectory. The resulting RMSD/time and Colormap plots can be found inside the `\texttt{RMSD-analysis/plots/}' folders. The trajectories are also provided inside the output `\texttt{*\_fep1/}' folder within the `\texttt{*\_complex\_10/}' directory.
\end{description}
\end{suppinfo}
