...
Here, we demonstrate the standard 5ns simulation time and default REST region selection are insufficient to allow for adequate sampling of the motion in the F-helix and does not eliminate the dependence on the initial protein state.  
...
In the case of moderate alchemical transformations, such as cases that involve the set of closed ligands (i.e. benzene to n-propylbenzene) to intermediate ligands (i.e. sec-/n-butylbenzene), we find that the calculated free energies still have a dependence on the initial protein conformation using the default protocol, albeit much smaller.
For this set of transformations to the intermediate state, the RMSE of the difference in $\Delta\Delta G_{calc}$ for protein closed versus open simulations is +0.60 kcal/mol (Table~\ref{tbl:C-I}), much closer to convergence than closed to open transformations.
...
From the protein closed trajectories, the RMSE from $\Delta\Delta G_{exp}$ is +1.33 kcal/mol (Table~\ref{tbl:C-nbutyl_closed}) while the protein open trajectories have a RMSE of +0.63 kcal/mol (Table~\ref{tbl:C-nbutyl_open}).
...
Primarily, we are encountering sampling issues when facing the challenge of opening the helix from the closed state.
In order to facilitate the protein motion, we included 3 key residues spanning the F-helix region into the REST region. %(see Methods for details?)
By expanding the REST region, we were able drive the F-helix out its initial state trap faster and effectively reduce our problem in inadequate sampling.
